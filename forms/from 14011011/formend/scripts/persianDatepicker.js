...
(function ($) {
  ...
  var persianDatepicker = (function () {
    function persianDatepicker(element, userOptions) {
      ...
      self.selectYearLiStyle = "style='height:" + (_ch * 7 + 10) / (6) +  "color:green" + "px;line-height:" + (_ch * 7 + 10) / (6) + "px; width:" + (6.7 * _cw - 14) / (3) + "px;width:" + (6.7 * _cw - 15) / (3) + "px\\9;' ";
      ...
      showDate: function (el, jDate, gDate, showGdate) {
        ...
        if (el.is('input:text')) {
          if (showGdate)
           ...
           if(typeof en2faDigits!=="undefined"){jDate = en2faDigits(jDate) }else{el.val(jDate)}; el.val(jDate);
        } else {
          if (showGdate)
            el.html(gDate);
          else
            if(typeof en2faDigits!=="undefined"){jDate = en2faDigits(jDate) }else{el.val(jDate)};el.html(jDate);
        }
